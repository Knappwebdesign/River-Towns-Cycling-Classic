<nav class="naver">
   <ul class="top-menu">
      <li><a href="roadraces.php">Road</a>
         <ul >
            <li>
               <a href="micromontour.php">Micro Mon-tour</a>
            </li>
            <li>
               <a href="minimontour.php">Mini Mon-tour</a>
            </li>
            <li>
               <a href="montour.php">Mon-Tour</a>
            </li>
            <li>
               <a href="crit.php">Criterium</a>
            </li>
            <li>
               <a href="timetrial.php">Time Trial</a>
            </li>
            <li>
               <a href="roadrides.php">Road Rides</a>
            </li>
         </ul>
      </li>

      <li><a href="mountainraces.php">Dirt</a>
         <ul>
            <li>
               <a href="hopewell.php">Hopewell Challenge<br />Mountain Bike Race</a>
            </li>
            <li>
               <a href="hillclimb.php">Hess Field Hill Climb</a>
            </li>
            <li>
               <a href="mountainrides.php">Mountain Rides</a>
            </li>
         </ul>
      </li>

      <li><a href="cyclocrossraces.php">Cyclocross</a>
         <ul>
            <li>
               <a href="meatybeaty.php">Meaty Beaty</a>
            </li>
            <li>
               <a href="boneshakercyclocross.php">Boneshaker CX</a>
            </li>
         </ul>
      </li>

      <li><a href="omnium.php">Omnium</a>
         <ul>
            <li>
               <a href="roadomnium.php">King and Queen of the Road</a>
            </li>
            <li>
               <a href="mountainomnium.php">Dirt Deity</a>
            </li>
         </ul>
      </li>

      <li><a href="expo.php">Expo</a>
         <ul>
            <li>
               <a href="mediacenter.php">Multi-Media Info Center</a>
            </li>
            <li>
               <a href="funstuff.php">Fun Stuff</a>
            </li>
            <li>
               <a href="vouchers.php">Vouchers</a>
            </li>
            <li>
               <a href="fallartsfestival.php">Fall Arts Festival</a>
            </li>
         </ul>
      </li>

      <li><a>Overview</a>
         <ul>
            <li>
               <a href="directions.php">Directions</a>
            </li>
            <li>
               <a href="registration.php">Registration</a>
            </li>
            <li>
               <a href="lodgingcamping.php">Lodging/Camping</a>
            </li>
            <li>
               <a href="schedule.php">Schedule Grid</a>
            </li>
            <li>
               <a href="events.php">Event Summary</a>
            </li>
            <li>
               <a href="faqs.php">FAQ's</a>
            </li>
            <li>
               <a href="contact.php">Contact Us</a>
            </li>
         </ul>
      </li>
   </ul>
</nav>
